...
public class MagicRuleParser extends magic.grammar.ParserMemo
{
  ...
  //=====================================================================
  //  IfCondition = "if a creature died this turn" ;
  //=====================================================================
  private boolean IfCondition()
    {
      if (saved(IfCondition)) return reuse();
      if (!next("if a creature died this turn")) return reject();
      return sem.action() && accept();
    }
  
  //=====================================================================
  //  AnimateAction = SelectPermanent " becomes a " CreatureSpec ;
  //=====================================================================
  private boolean AnimateAction()
    {
      if (saved(AnimateAction)) return reuse();
      if (!SelectPermanent()) return reject();
      if (!next(" becomes a ")) return reject();
      if (!CreatureSpec()) return reject();
      return sem.action() && accept();
    }
  
  //=====================================================================
  //  AnimateActionReminder = "it's still a land" ;
  //=====================================================================
  private boolean AnimateActionReminder()
    {
      if (saved(AnimateActionReminder)) return reuse();
      if (!next("it's still a land")) return reject();
      return sem.action() && accept();
    }
  ...
  //    unblockable" / "are unblockable" / "attacks each turn if able" /
  ...
  private boolean PermanentState()
    {
      ...
      if (next("is indestructible")) return sem.action() && accept();
      if (next("are indestructible")) return sem.action() && accept();
      if (next("is unblockable")) return sem.action() && accept();
      if (next("are unblockable")) return sem.action() && accept();
  ...
  //=====================================================================
  //  CreatureSpec = PowerToughness (SPACE ColorSpec)? (SPACE
  //    CreatureType)* " artifact"? " creature" (SPACE WithAbilitySpec)?
  //    ;
  //=====================================================================
  ...
  //-------------------------------------------------------------------
  //  CreatureSpec_0 = SPACE ColorSpec
  //-------------------------------------------------------------------
  ...
  //=====================================================================
  //  SpellRestriction = ColorSpec / "creature" / "multicolored" /
  //    "noncreature" / "enchantment" / "spirit or arcane" / "treefolk" /
  //    "vampire creature" / "wizard" / "artifact" / "during an
  //    opponent's turn" / "instant or sorcery" / "instant" / "sorcery" /
  //    "with infect" / "with converted mana cost " Number " or less" /
  ...
  private boolean SpellRestriction()
    {
      ...
      if (next("creature")) return sem.action() && accept();
      if (next("multicolored")) return sem.action() && accept();
      if (next("noncreature")) return sem.action() && accept();
      if (next("enchantment")) return sem.action() && accept();
      if (next("spirit or arcane")) return sem.action() && accept();
      if (next("treefolk")) return sem.action() && accept();
      if (next("vampire creature")) return sem.action() && accept();
      ...
      if (next("instant or sorcery")) return sem.action() && accept();
  ...
  //    "blocked" / "equipped" / Tribal / PermanentRestriction /
  //    PowerToughness ;
  //=====================================================================
  ...
  private boolean CreatureRestriction_3()
    {
      ...
      CreatureRestriction_4();
      if (!next("counter on it")) return rejectInner();
      return acceptInner();
    }
  
  //-------------------------------------------------------------------
  //  CreatureRestriction_4 = CounterType SPACE
  //-------------------------------------------------------------------
  ...
  final Cache CreatureSpec_0 = new Cache("CreatureSpec_0"); // SPACE ColorSpec
  ...
  final Cache CreatureRestriction_4 = new Cache("CreatureRestriction_4"); // CounterType SPACE
  ...
  Cache[] cacheList =
  {
    ...
    AbilityRestriction,IfCondition,AnimateAction,
    AnimateActionReminder,ChangeLoyaltyAction,ChangeStateAction,
    PermanentState,SetPTAction,SacrificeAction,PoisonAction,
    MillAction,DoesntUntapAction,ShuffleAction,PutTokenAction,
    CreatureTokenSpec,WithAbilitySpec,CreatureSpec,ControlAction,
    ChangeCounterAction,CounterVerb,Counter,PayManaAction,
    ChangeLifeAction,ChangeLifeVerb,SelectPlayer,Player,PutCardAction,
    ReturnAction,BounceAction,BounceLocation,BounceVerb,PreventAction,
    DamageRestriction,DamageVerb,Damage,TapAction,TapVerb,
    RegenerateAction,DamageAction,Deal,EntersTapped,EntersWithCounter,
    CounterType,Count,Enchant,Duration,GainAction,GainVerb,PumpAction,
    AddManaAction,ManaSource,DrawAction,Draw,DiscardAction,Discard,
    DestroyNoRegenAction,DestroyAction,ExileAction,CounterAction,
    SelectOp,SelectCard,SelectCreature,SelectPermanent,SelectSpell,
    ...
    LandRestriction,Color,Tribal,CreatureType,Creature,Artifact,Land,
    ...
    Keyword_20,Keyword_21,Keyword_22,ColorlessCost_0,ManaCost_0,
    ManaCost_1,ManaCost_2,CastTrigger_0,CastTrigger_1,
    EntersBattlefieldTrigger_0,DiesTrigger_0,DiesTrigger_1,
    DealsDamageTrigger_0,DamageReceiver_0,DamageReceiver_1,
    DamageReceiver_2,DamageReceiver_3,DamageReceiver_4,
    DamageReceiver_5,DamageReceiver_6,ChangeStateAction_0,
    PermanentState_0,PermanentState_1,SetPTAction_0,SetPTAction_1,
    SacrificeAction_0,MillAction_0,DoesntUntapAction_0,
    DoesntUntapAction_1,DoesntUntapAction_2,PutTokenAction_0,
    PutTokenAction_1,CreatureTokenSpec_0,CreatureTokenSpec_1,
    WithAbilitySpec_0,WithAbilitySpec_1,CreatureSpec_0,
