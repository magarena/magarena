...
public class CardFilterPanel extends TexturedPanel implements ActionListener {
	...
	private final String[] FILTER_CHOICES = { UiString.get(_S1), UiString.get(_S2), UiString.get(_S3) };
	...
	private boolean disableUpdate; // so when we change several filters, it
									// doesn't update until the end

	public CardFilterPanel(final ICardFilterPanelListener aListener) {

		this.listener = aListener;

		cardPool = listener.isDeckEditor() ? CardDefinitions.getDefaultPlayableCardDefs()
	...
	private void populateCheckboxPopup(final String migLayout, final ButtonControlledPopup popup,
			final Object[] checkboxValues, final JCheckBox[] newCheckboxes, final JRadioButton[] newFilterButtons,
	...
	private void populateCheckboxPopup(final ButtonControlledPopup popup, final Object[] checkboxValues,
			final JCheckBox[] newCheckboxes, final JRadioButton[] newFilterButtons, final boolean hideAND) {
	...
	private boolean isCardfiltered(final MagicCardDefinition cardDefinition) {
		...
		// format
		if (!filterCheckboxes(cardDefinition, formatsCheckBoxes, formatsFilterChoices, new CardChecker() {
			@Override
		...
		// sets
		if (!filterCheckboxes(cardDefinition, setsCheckBoxes, setsFilterChoices, new CardChecker() {
			@Override
			public boolean checkCard(final MagicCardDefinition card, final int i) {
		...
		// type
		if (!filterCheckboxes(cardDefinition, typeCheckBoxes, typeFilterChoices, new CardChecker() {
			@Override
			public boolean checkCard(final MagicCardDefinition card, final int i) {
		...
		// color
		if (!filterCheckboxes(cardDefinition, colorCheckBoxes, colorFilterChoices, new CardChecker() {
			@Override
			public boolean checkCard(final MagicCardDefinition card, final int i) {
		...
		// cost
		if (!filterCheckboxes(cardDefinition, costCheckBoxes, costFilterChoices, new CardChecker() {
			@Override
			public boolean checkCard(final MagicCardDefinition card, final int i) {
		...
		// subtype
		if (!filterCheckboxes(cardDefinition, subtypeCheckBoxes, subtypeFilterChoices, new CardChecker() {
			@Override
			public boolean checkCard(final MagicCardDefinition card, final int i) {
		...
		// rarity
		if (!filterCheckboxes(cardDefinition, rarityCheckBoxes, rarityFilterChoices, new CardChecker() {
			@Override
			public boolean checkCard(final MagicCardDefinition card, final int i) {
		...
		// status
		if (!filterCheckboxes(cardDefinition, statusCheckBoxes, statusFilterChoices, new CardChecker() {
			@Override
			public boolean checkCard(final MagicCardDefinition card, final int i) {
	...
	private boolean filterCheckboxes(final MagicCardDefinition cardDefinition, final JCheckBox[] checkboxes,
			final JRadioButton[] filterButtons, final CardChecker func) {
	...
	private Stream<MagicCardDefinition> getFilteredStream() {
		return listener.isDeckEditor() ? cardPool.stream().filter(c -> !c.isHidden() && isCardfiltered(c))
	...
	private void updateCardTotals(final List<MagicCardDefinition> cards) {
		...
		missingCards = (int) cards.stream().filter(c -> c.isPlayable() && c.isInvalid()).count();

		playableCards = (int) cards.stream().filter(c -> c.isPlayable() && !c.isInvalid()).count();
	}
	...
	private void addCardTypeFilter() {
		...
		populateCheckboxPopup("flowy, wrap 9, insets 2, gapy 6, gapx 20", typePopup, types, typeCheckBoxes,
				typeFilterChoices, false);
	}
	...
	private void addManaCostFilter() {
		...
		populateCheckboxPopup("flowx, wrap 5, insets 2, gap 8", costPopup, COST_VALUES, costCheckBoxes,
				costFilterChoices, true);
	}
