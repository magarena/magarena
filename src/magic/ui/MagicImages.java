...
public final class MagicImages {
	...
	public static BufferedImage getOrigSizeCardImage(MagicCardDefinition aCard, JComponent jcomp) {
		...
		//final Throwable debug=new RuntimeException("debug");
		new Thread() {
			public void run() {// download image on demand
				if (proxy == null)
	...
	public static BufferedImage geCardImageUseCache(MagicCardDefinition aCard, JComponent jcomp) {
		int key = aCard.getIndex();
		if (cache.containsKey(key)) {
			return cache.get(key);
		}
		BufferedImage image = getOrigSizeCardImage(aCard, jcomp);
