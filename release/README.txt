...
- Use (slightly) better quality scaling for images in ImageCardListViewer (closes #545).
- Prepare game in background of duel decks screen and only enable play button when ready (see #566).
- Update mulligan screen to clarify it is using the Vancouver rule, add wiki button (see #557).
- Wiki button and on-screen hints added to download dialog.
- Download image folder setting moved to download dialog.
- New option on download dialog lets you choose preferred language for card text (see #558).
- Add preferred image size presets for in-game popup to preferences dialog (see #527).
- Make card popup fade-in optional (closes #570).
- New animations and effects - draw from library; play-from arrows; turn card over on play/draw.

    Rename Colorless MagicCostManaType to Generic and add Colorless {C}
    remove CardProxyImageBuilder - proxy images now generated by CardBuilder.
    Loads game in background when DuelDecksScreen is active (see issue #566).
    show ability icons for all permanents instead of only for creatures
    Flip card from back to front on play; Play-from arrow indicators.
    Update animation settings tab to include new animations and fx.
    both sides of a double-sided card are shown during a play/draw flip animation.
    Preventing saving user deck to Firemind folder as well as prebuilt.
    Simplify in-game images settings (see #527).
    allow conditional effects in rider, fixes #509
    add effect 'you get <amount> poison counters.'
    support 'each player's end step' trigger
    support trigger 'at the beginning of each opponent's upkeep'
    add trigger 'when <perm> attacks and isn't blocked'
    add 'when <creature> attacks alone'
    add ExileGroupCard
    add effect '<source> deals X damage to <targets>, where X is <amount>'
    ...
    add M logo.
    at end of mulligan phase, players scry 1 if they have less cards than their starting hand, closes #557
    Update mulligan screen to clarify it is using the Vancouver rule (see #557).
    make strings translatable.
    Use better quality scaling for images in ImageCardListViewer (closes #545).
...
- fixed the following bugs:
    ...
    fix: was not picking up change in card text language setting during same session.
    Clear image cache on closing or hiding download images screen so that new image is displayed instead of out-of-date version.
    play-card-from-hand animation now takes into account whether card is being played face up or down.
    Clicking on card name in log displays correct face-up/down image in popup with modified PT and ability icons (see #567).
    Clicking on card name in stack  displays correct face-up/down image in popup (see #567).
    only construct missing image if actual image cannot be loaded
    fix Plague Rats to use the correct quoting character of backtick instead of double quote
    edit Lich and Nefarious Lich game loss triggers to use the stack
    Transcendence state trigger should go onto the stack like any other state trigger
    add MagicCard.isPermanentCard, check that card is a permanent card before putting it on the battlefield, fixes #554
    add May tense for 'PN may PN draws' fix
    wrap MagicCondition.ABILITY_TWICE_CONDITION in MagicArtificialCondition so that is does not affect human player, fixes #551
    fix missing mana_or_combat for Lumbering Falls and Shambling Vent
    fix missing mana_or_combat property for Izzet Keyrune
    fixes #535 : Fix poor quality scaling of card image during preview stage of animation
    fixes #540 : ArrayIndexOutOfBoundsException: CardsCanvas.highlightCardUnderMousePointer.
    only triggers get MagicEvent.NONE when condition check fails, fixes #519
    fixes #529 : NPE if escape key pressed on game loading screen.
    add line listener that closes clip on stop to release resources, for #526
    edit GainLifeYou to only display life gain total for 'for each' fixes #534
    edit DiscardChosen to only display discard total for 'for each'
    edit LifeLoseYou to only display loss total for 'for each'
    prevent exception if json feed has a null string. (fixes  #563).
